...
// The banner restores web globals the host strips; the text loaders inline the
// composition and GSAP so we never read our own install dir at runtime.
...
await esbuild.build({
  ...
  loader: { '.html': 'text', '.txt': 'text' }, // inline the composition + GSAP as strings
  banner: { js: banner },
  logLevel: 'info',
});
